...
function buildWorkspaceCommandDefinition(input) {
    return {
        id: deriveWorkspaceCommandId({
            ...
            name: readNonEmptyString(input.entry.name)
...
function uniqueWorkspaceCommandId(seen, commandId, sourceKey, sourceIndex) {
...
function readCommandEntries(workspaceRuntime, key) {
    const raw = workspaceRuntime?.[key];
    return Array.isArray(raw) ? raw.filter((entry) => isRecord(entry)) : [];
}
export function listWorkspaceCommandDefinitions(workspaceRuntime) {
    if (!workspaceRuntime)
        return [];
    ...
    if (commandEntries.length > 0) {
        return commandEntries.map((entry, index) => finalize(buildWorkspaceCommandDefinition({
            entry,
    ...
    const serviceDefinitions = readCommandEntries(workspaceRuntime, "services").map((entry, index) => finalize(buildWorkspaceCommandDefinition({
        entry,
        kind: "service",
        sourceKey: "services",
        sourceIndex: index,
    ...
    const jobDefinitions = readCommandEntries(workspaceRuntime, "jobs").map((entry, index) => finalize(buildWorkspaceCommandDefinition({
        entry,
        kind: "job",
        sourceKey: "jobs",
        sourceIndex: index,
...
export function listWorkspaceServiceCommandDefinitions(workspaceRuntime) {
    return listWorkspaceCommandDefinitions(workspaceRuntime).filter((command) => command.kind === "service");
}
export function findWorkspaceCommandDefinition(workspaceRuntime, workspaceCommandId) {
...
export function scoreWorkspaceRuntimeServiceMatch(command, runtimeService) {
    ...
    let score = 0;
    if (runtimeService.serviceName === command.name)
        score += 4;
    if ((runtimeService.command ?? null) === (command.command ?? null))
        score += 4;
    if (command.cwd
        && runtimeService.cwd
...
export function matchWorkspaceRuntimeServiceToCommand(command, runtimeServices) {
    let bestMatch = null;
    let bestScore = -1;
